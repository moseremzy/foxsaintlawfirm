...
const ContactData = () => {
          ...
          >
           {contactinfo.map((item,index)=>(
            <section key={index} className='flex flex-col items-center md:flex-row '>
                <div className='w-full flex flex-col shadow-gray-300 shadow-lg p-3 mb-5 rounded-xl md:shadow-none md: justify-center items-center md:items-end'>
                    <h2 className='text-[17px] text-gray-600 md:text-3xl'>{item.interest}</h2>
                    <h1 className='text-3xl text-[#c4a85c] font-bold pb-5 md:text-3xl'>{item.team}</h1>
                    <article className='flex flex-col gap-2 pb-3 font-semibold text-gray-600 text-[16px] md:text-xl'>
                        <p className='text-center md:text-end'>{item.description}</p>
                        <p className='text-center md:text-end'>{item.text}</p>
                    </article>
                </div>
  ...
  <input type="file" class="hidden" />
                ...
                <button type="submit" className='w-full border-1 p-2 bg-[#D3B45E] active:bg-[#D3B45E] text-white cursor-pointer md:hover:bg-[#fcca41]'>Submit</button>
            </form>
           </div>
            </section>
           ))}
