...
<div className='block md:hidden'>
  ...
  <section className='bg-[#16163F] flex flex-col gap-5 p-6 w-full justify-center items-start'>
    ...
    <p className='font-semibold'>We take pride in our expertise across various areas of law.</p>
    <ul>
      {practiceAreas.map((item, index) => (
        <li key={index} className='flex flex-col py-3 text-lg justify-center items-start border-b border-gray-300 w-full last:border-b-0'>
...
<div className='hidden md:flex flex-row-reverse w-full'>
  ...
  <section className='bg-[#16163F] flex flex-col gap-5 px-20 py-5 w-1/2 justify-center items-start text-white'>
    ...
    <ul className='w-full'>
      {practiceAreas.map((item, index) => (
        <li key={index} className='flex flex-col py-3 text-lg justify-center items-start border-b border-gray-300 w-120 last:border-b-0'>
          <a href={item.href} className='flex flex-row items-center gap-2'>
            <span className='text-amber-500'>{item.icon}</span>
